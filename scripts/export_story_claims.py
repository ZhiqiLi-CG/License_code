...
from license_to_act.story_claims import write_story_claims  # noqa: E402


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Export License-to-Act story claim/evidence files.")
    ...
    claims = write_story_claims(
        project_root(),
        paper_data_dir=args.paper_data_dir,
        paper_sections_dir=args.paper_sections_dir,
        summary_path=args.summary,
    )
    print(claims["outputs"]["summary_json"])
    print(claims["outputs"]["claims_csv"])
    print(claims["outputs"]["latex_numbers"])
    return 0
