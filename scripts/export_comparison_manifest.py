...
from license_to_act.comparison_manifest import write_comparison_manifest  # noqa: E402
from license_to_act.paths import artifact_path, project_root  # noqa: E402


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Export the action-boundary comparison manifest.")
    ...
    parser.add_argument(
        "--summary",
        type=Path,
        default=artifact_path("paper_results", "lta_comparison_manifest_20260831.json"),
    )
    ...
    print(manifest["outputs"]["summary_json"])
    print(manifest["outputs"]["manifest_csv"])
    print(manifest["outputs"]["latex_numbers"])
    return 0
