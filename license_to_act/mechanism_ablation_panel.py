...
def build_mechanism_ablation_panel(project_root: str | Path = Path("/data/zhiqi/License")) -> dict[str, Any]:
    ...
    rows = [
        _row(
            "ABLATE_POLICY_AUTHORIZATION_EVIDENCE",
            "no",
            "PolicyAuthorizationEvidence",
            "User intent and reservation evidence do not authorize business-record writes.",
            ["T2-A1", "T2-A48"],
        ...
        _row(
            "ABLATE_SIDE_EFFECT_BOUNDS",
            "yes",
            "denied regions and forbidden_side_effects",
            "Local task success can mutate collateral Git state without side-effect authority.",
            ["TB-SAN"],
        ...
        _row(
            "ABLATE_READ_LICENSE",
            "no",
            "read_license for fragile evidence",
            "Observation can consume the recovery substrate unless reads are licensed.",
            ["TB-WAL"],
        ...
        _row(
            ...
            "yes",
            "OBLIGE transitions for verifier-visible artifacts",
            "Complete evidence is insufficient when no runtime obligation owns the output commit.",
            ["SF-INV", "SF-QWEN32K-MSWE-2"],
            _add_counts(_stage1_cut(stage1, ["SF-INV"]), _stage2_cut(stage2, ["SF-QWEN32K-MSWE-2"])),
        ...
        _row(
            ...
            "yes",
            "GovKernel runtime commit control",
            "Natural-language license text does not substitute for owning the durability boundary.",
            ["SF-INV"],
...
def _row(
    ...
    return {
        ...
        "paper_result": f"{cut_passes}/{cut_trials} cut pass versus {full_passes}/{full_trials} full LTA pass",
